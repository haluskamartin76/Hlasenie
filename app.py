...
try:
    ...
    # Otvárame priamo cez URL, čo je najistejšie
    sh = client.open_by_url(SPREADSHEET_URL).sheet1
except Exception as e:
    st.error(f"Chyba pripojenia: {type(e).__name__} - {e}")
    st.stop()
...
with t1:
    with st.form("form_pracovisko", clear_on_submit=True):
        ...
        if st.form_submit_button("Odoslať"):
            ...
            st.success("Hlásenie uložené v Google Sheets!")
